window.TsChat = function MegaSuperUsefulChat(userConfig) {
  ...
  var dbEndpoints = {
    ...
    userStatActive: "statistics/active",
    userStatMsgCount: "statistics/msgs"
  };
  var ALL_SETTINGS_ENDPOINT = dbEndpoints.userSettings + '/' + config.uuid + '.json';
  var ALL_STATISTICS_ENDPOINT = dbEndpoints.userStatistics + '/' + config.uuid + '.json';
  ...
  var POFILE_ACTIVE_ENDPOINT = dbEndpoints.userProfile + '/' + config.uuid + '/' + dbEndpoints.userStatActive + '.json';
  var POFILE_MSGS_COUNT_ENDPOINT = dbEndpoints.userProfile + '/' + config.uuid + '/' + dbEndpoints.userStatMsgCount + '.json';
  ...
  var UserSettings = function UserSettings0(uuid, maximized, name) {
    this.uuid = uuid;
    this.maximized = maximized;
    this.name = name;
  };
  var UserStatistics = function UserStatActive0(active, msgs) {
    this.active = active;
    this.msgs = msgs;
  };
  var UserStatActive = function UserStatActive0(date) {
    this.lastSeen = date;
  };
  ...
  function saveMessageToStorage(record, updateConfig) {
    if (updateConfig) {
  ...
  function clickOnUserSendButton() {
    ...
    if (config.loadCompleted) {
      if (msg !== '') {
        ...
        saveMessageToStorage(record, true);
    ...
    if (!config.nameVerified) {
      if (userSettings.name !== undefined && userSettings.name !== null && userSettings.name !== "") {
        ...
        textArea.addEventListener('focus', function inputName() {
          ...
          config.nameVerified = true;
          updatedSettings = new UserSettings(config.uuid, config.maximized, config.userName);
          saveObjectToRemoteDatabase(PROFILE_SETTINGS_ENDPOINT, updatedSettings);
        });
      }
    }
    saveObjectToRemoteDatabase(ALL_SETTINGS_ENDPOINT, userSettings);
  }
  ...
  function processUserProfileFromStorage(profile) {
    ...
    if (profile !== undefined && profile !== null) {
      ...
      defaultSettings = new UserSettings(config.uuid, config.maximized, config.userName);
  ...
  window.addEventListener('load', function InitializeMegaSuperUsefulChat() {
    readUserConfig();
    renderChatContainer();
    setInterval(syncTranscriptAndStatistics, 10 * 1000);
    // syncTranscriptAndStatistics();
  });
};